...
%% WriteToFile        
        function writeLog(self,level,scriptName,message)
            shouldPrintToCommandWindow = self.commandWindowLevel <= level;
            shouldWriteToLog                        = self.logLevel <= level;
            
            if( (~shouldPrintToCommandWindow) && (~shouldWriteToLog) )
                return;
            end
